...
When reviewing manifest.yaml, systematically verify ALL items:

1. **Template Defaults** - Check: `name: Todo`, `external_system_name: Todo`
2. **Function Mismatches** - Verify functions.name matches imports references
3. **Missing Connection Types** - Ensure allowed_connection_types references defined keyrings
4. **Non-Unique organization_data** (CRITICAL) - Check for workspace_name, team names, public email domains
5. **Wrong is_subdomain** - Verify `true` has [SUBDOMAIN], `false` has organization_data
