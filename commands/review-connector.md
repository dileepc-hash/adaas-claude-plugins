...
allowed-tools:
  ...
  - Glob
  - Bash
argument-hint: "[--phases=phase1,phase2]"
---
...
2. **Parse arguments**: Extract `--phases` filter if provided
...
5. **Generate findings**: Output flat list of actionable issues to address
...
Use the **Review Findings Format** defined in the skill SKILL.md (lines 54-79):
...
- Only report issues that need to be fixed
- No summary sections or counts
- No positive findings
- Focus on actionable changes

See SKILL.md for complete format specification and examples.
...
Review all phases. Check manifest, package.json, all worker files, http-client, normalization, and cross-cutting concerns.
...
```
User: /review-connector --phases=security
```

Review only security checklist, report actionable security issues.
...
8. Generate flat list of actionable issues

Focus on actionable fixes only. Explain what's wrong and how to fix it, without categorizing by severity or providing summary statistics.
